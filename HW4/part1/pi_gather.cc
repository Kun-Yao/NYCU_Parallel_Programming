...
int main(int argc, char **argv)
{
    ...
    int dest = 0;
    int tag=0;
    long long int* recv_data = (long long int*)calloc(world_size, sizeof(long long int));
    MPI_Status status;
    ...
    // TODO: use MPI_Gather
    MPI_Gather(&local_count, 1, MPI_LONG_LONG_INT, recv_data, 1, MPI_LONG_LONG_INT, 0, MPI_COMM_WORLD);
    if (world_rank == 0)
    {
        // TODO: PI result
        long long int global_count = 0;
        for(int i = 0; i < world_size; i++){
            global_count += recv_data[i];
        }
